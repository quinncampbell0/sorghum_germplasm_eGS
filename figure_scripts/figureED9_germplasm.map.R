...
own_accs <- read.csv('./input_data/network_map_own_accessions.csv')[,-1]
...
full_dat <- all_full_dat[which(all_full_dat$n > 10),]
...
map('world',col="black", fill=FALSE, bg="white", lwd=1,mar=rep(0,4), ylim=c(-90,90) )
...
gradientLegend(c(0, 40000), color = col_points, side = 3, length = 0.25, n.seg = 4)
n_legend <- c(40000, 10000, 1000, 100)
scale_legend = log(n_legend, base = 2)
legend(x = "center", legend = c("40,000", "10,000", "1,000", "100"), pch=20, pt.cex = scale_legend, col = 'darkblue', 
       y.intersp = 4, x.intersp = 3, title = "# of Accessions Held by Genebank")
plot.new()

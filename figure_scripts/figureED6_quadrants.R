...
p_C <- ggplot(out_up, aes(x = GEAV, y = overlap.perc, color = pos)) +
  geom_point() +
  ylim(0,100) +
  scale_color_manual(
...
p_D <- ggplot(geav_envdat, aes(y = GEAV, x = as.factor(pos))) +
  geom_boxplot() +
...
pdf('./output_figures/quadrant_graphs.pdf', width = 8.5, height = 11)
ggarrange(p_A, p_B, p_C, p_D, ncol = 1, labels = c('A','B','C','D'), heights = c(1,1,1.5,1.3))
dev.off()
